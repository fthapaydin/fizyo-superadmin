export const TURKEY_CITIES = [
  {
    name: 'İstanbul',
    districts: [
      'Adalar', 'Arnavutköy', 'Ataşehir', 'Avcılar', 'Bağcılar', 'Bahçelievler', 'Bakırköy', 'Başakşehir',
      'Bayrampaşa', 'Beşiktaş', 'Beykoz', 'Beylikdüzü', 'Beyoğlu', 'Büyükçekmece', 'Çatalca', 'Çekmeköy',
      'Esenler', 'Esenyurt', 'Eyüpsultan', 'Fatih', 'Gaziosmanpaşa', 'Güngören', 'Kadıköy', 'Kağıthane',
      'Kartal', 'Küçükçekmece', 'Maltepe', 'Pendik', 'Sancaktepe', 'Sarıyer', 'Silivri', 'Sultanbeyli',
      'Sultangazi', 'Şile', 'Şişli', 'Tuzla', 'Ümraniye', 'Üsküdar', 'Zeytinburnu'
    ]
  },
  {
    name: 'Ankara',
    districts: [
      'Akyurt', 'Altındağ', 'Ayaş', 'Bala', 'Beypazarı', 'Çamlıdere', 'Çankaya', 'Çubuk', 'Elmadağ',
      'Etimesgut', 'Evren', 'Gölbaşı', 'Güdül', 'Haymana', 'Kahramankazan', 'Kalecik', 'Keçiören',
      'Kızılcahamam', 'Mamak', 'Nallıhan', 'Polatlı', 'Pursaklar', 'Sincan', 'Şereflikoçhisar', 'Yenimahalle'
    ]
  },
  {
    name: 'İzmir',
    districts: [
      'Aliağa', 'Balçova', 'Bayındır', 'Bayraklı', 'Bergama', 'Beydağ', 'Bornova', 'Buca', 'Çeşme',
      'Çiğli', 'Dikili', 'Foça', 'Gaziemir', 'Güzelbahçe', 'Karabağlar', 'Karaburun', 'Karşıyaka',
      'Kemalpaşa', 'Kınık', 'Kiraz', 'Konak', 'Menderes', 'Menemen', 'Narlıdere', 'Ödemiş', 'Seferihisar',
      'Selçuk', 'Tire', 'Torbalı', 'Urla'
    ]
  },
  {
    name: 'Bursa',
    districts: [
      'Büyükorhan', 'Gemlik', 'Gürsu', 'Harmancık', 'İnegöl', 'İznik', 'Karacabey', 'Keles', 'Kestel',
      'Mudanya', 'Mustafakemalpaşa', 'Nilüfer', 'Orhaneli', 'Orhangazi', 'Osmangazi', 'Yenişehir', 'Yıldırım'
    ]
  },
  {
    name: 'Antalya',
    districts: [
      'Akseki', 'Aksu', 'Alanya', 'Demre', 'Döşemealtı', 'Elmalı', 'Finike', 'Gazipaşa', 'Gündoğmuş',
      'İbradı', 'Kaş', 'Kemer', 'Kepez', 'Konyaaltı', 'Korkuteli', 'Kumluca', 'Manavgat', 'Muratpaşa', 'Serik'
    ]
  },
  {
    name: 'Adana',
    districts: ['Aladağ', 'Ceyhan', 'Çukurova', 'Feke', 'İmamoğlu', 'Karaisalı', 'Karataş', 'Kozan', 'Pozantı', 'Saimbeyli', 'Sarıçam', 'Seyhan', 'Tufanbeyli', 'Yumurtalık', 'Yüreğir']
  },
  {
    name: 'Konya',
    districts: ['Ahırlı', 'Akören', 'Akşehir', 'Altınekin', 'Beyşehir', 'Bozkır', 'Cihanbeyli', 'Çeltik', 'Çumra', 'Derbent', 'Derebucak', 'Doğanhisar', 'Emirgazi', 'Ereğli', 'Güneysınır', 'Hadim', 'Halkapınar', 'Hüyük', 'Ilgın', 'Kadınhanı', 'Karapınar', 'Karatay', 'Kulu', 'Meram', 'Sarayönü', 'Selçuklu', 'Seydişehir', 'Taşkent', 'Tuzlukçu', 'Yalıhüyük', 'Yunak']
  },
  {
    name: 'Gaziantep',
    districts: ['Araban', 'İslahiye', 'Karkamış', 'Nizip', 'Nurdağı', 'Oğuzeli', 'Şahinbey', 'Şehitkamil', 'Yavuzeli']
  },
  {
    name: 'Kocaeli',
    districts: ['Başiskele', 'Çayırova', 'Darıca', 'Derince', 'Dilovası', 'Gebze', 'Gölcük', 'İzmit', 'Kandıra', 'Karamürsel', 'Kartepe', 'Körfez']
  },
  {
    name: 'Mersin',
    districts: ['Akdeniz', 'Anamur', 'Aydıncık', 'Bozyazı', 'Çamlıyayla', 'Erdemli', 'Gülnar', 'Mezitli', 'Mut', 'Silifke', 'Tarsus', 'Toroslar', 'Yenişehir']
  },
  {
    name: 'Diyarbakır',
    districts: ['Bağlar', 'Bismil', 'Çermik', 'Çınar', 'Çüngüş', 'Dicle', 'Eğil', 'Ergani', 'Hani', 'Hazro', 'Kayapınar', 'Kocaköy', 'Kulp', 'Lice', 'Silvan', 'Sur', 'Yenişehir']
  },
  {
    name: 'Kayseri',
    districts: ['Akkışla', 'Bünyan', 'Develi', 'Felahiye', 'Hacılar', 'İncesu', 'Kocasinan', 'Melikgazi', 'Özvatan', 'Pınarbaşı', 'Sarıoğlan', 'Sarız', 'Talas', 'Tomarza', 'Yahyalı', 'Yeşilhisar']
  },
  {
    name: 'Eskişehir',
    districts: ['Alpu', 'Beylikova', 'Çifteler', 'Günyüzü', 'Han', 'İnönü', 'Mahmudiye', 'Mihalgazi', 'Mihalıççık', 'Odunpazarı', 'Seyitgazi', 'Sivrihisar', 'Tepebaşı']
  },
  {
    name: 'Samsun',
    districts: ['Alaçam', 'Asarcık', 'Atakum', 'Ayvacık', 'Bafra', 'Canik', 'Çarşamba', 'Havza', 'İlkadım', 'Kavak', 'Ladik', 'Ondokuzmayıs', 'Salıpazarı', 'Tekkeköy', 'Terme', 'Vezirköprü', 'Yakakent']
  },
  {
    name: 'Denizli',
    districts: ['Acıpayam', 'Babadağ', 'Baklan', 'Bekilli', 'Beyağaç', 'Bozkurt', 'Buldan', 'Çal', 'Çameli', 'Çardak', 'Çivril', 'Güney', 'Honaz', 'Kale', 'Merkezefendi', 'Pamukkale', 'Sarayköy', 'Serinhisar', 'Tavas']
  },
  {
    name: 'Şanlıurfa',
    districts: ['Akçakale', 'Birecik', 'Bozova', 'Ceylanpınar', 'Eyyübiye', 'Halfeti', 'Haliliye', 'Harran', 'Hilvan', 'Karaköprü', 'Siverek', 'Suruç', 'Viranşehir']
  },
  {
    name: 'Adapazarı (Sakarya)',
    districts: ['Adapazarı', 'Akyazı', 'Arifiye', 'Erenler', 'Ferizli', 'Geyve', 'Hendek', 'Karapürçek', 'Karasu', 'Kaynarca', 'Kocaali', 'Pamukova', 'Sapanca', 'Serdivan', 'Söğütlü', 'Taraklı']
  },
  {
    name: 'Trabzon',
    districts: ['Akçaabat', 'Araklı', 'Arsin', 'Beşikdüzü', 'Çarşıbaşı', 'Çaykara', 'Dernekpazarı', 'Düzköy', 'Hayrat', 'Köprübaşı', 'Maçka', 'Of', 'Ortahisar', 'Sürmene', 'Şalpazarı', 'Tonya', 'Vakfıkebir', 'Yomra']
  },
  {
    name: 'Muğla',
    districts: ['Bodrum', 'Dalaman', 'Datça', 'Fethiye', 'Kavaklıdere', 'Köyceğiz', 'Marmaris', 'Menteşe', 'Milas', 'Ortaca', 'Seydikemer', 'Ula', 'Yatağan']
  },
  {
    name: 'Tekirdağ',
    districts: ['Çerkezköy', 'Çorlu', 'Ergene', 'Hayrabolu', 'Kapaklı', 'Malkara', 'Marmaraereğlisi', 'Muratlı', 'Saray', 'Süleymanpaşa', 'Şarköy']
  },
  {
    name: 'Balıkesir',
    districts: ['Altıeylül', 'Ayvalık', 'Balya', 'Bandırma', 'Bigadiç', 'Burhaniye', 'Dursunbey', 'Edremit', 'Erdek', 'Gömeç', 'Gönen', 'Havran', 'İvrindi', 'Karesi', 'Kepsut', 'Manyas', 'Marmara', 'Savaştepe', 'Sındırgı', 'Susurluk']
  },
  {
    name: 'Manisa',
    districts: ['Ahmetli', 'Akhisar', 'Alaşehir', 'Demirci', 'Gölmarmara', 'Gördes', 'Kırkağaç', 'Köprübaşı', 'Kula', 'Salihli', 'Sarıgöl', 'Saruhanlı', 'Selendi', 'Soma', 'Şehzadeler', 'Turgutlu', 'Yunusemre']
  },
  {
    name: 'Aydın',
    districts: ['Bozdoğan', 'Buharkent', 'Çine', 'Didim', 'Efeler', 'Germencik', 'İncirliova', 'Karacasu', 'Karpuzlu', 'Koçarlı', 'Köşk', 'Kuşadası', 'Kuyucak', 'Nazilli', 'Söke', 'Sultanhisar', 'Yenipazar']
  },
  {
    name: 'Hatay',
    districts: ['Altınözü', 'Antakya', 'Arsuz', 'Belen', 'Defne', 'Dörtyol', 'Erzin', 'Hassa', 'İskenderun', 'Kırıkhan', 'Kumlu', 'Payas', 'Reyhanlı', 'Samandağ', 'Yayladağı']
  },
  {
    name: 'Kahramanmaraş',
    districts: ['Afşin', 'Andırın', 'Çağlayancerit', 'Dulkadiroğlu', 'Ekinözü', 'Elbistan', 'Göksun', 'Nurhak', 'Onikişubat', 'Pazarcık', 'Türkoğlu']
  },
  {
    name: 'Malatya',
    districts: ['Akçadağ', 'Arapgir', 'Arguvan', 'Battalgazi', 'Darende', 'Doğanşehir', 'Doğanyol', 'Hekimhan', 'Kale', 'Kuluncak', 'Pütürge', 'Yazıhan', 'Yeşilyurt']
  },
  {
    name: 'Erzurum',
    districts: ['Aşkale', 'Aziziye', 'Çat', 'Hınıs', 'Horasan', 'İspir', 'Karaçoban', 'Karayazı', 'Köprüköy', 'Narman', 'Oltu', 'Olur', 'Palandöken', 'Pasinler', 'Pazaryolu', 'Şenkaya', 'Tekman', 'Tortum', 'Uzundere', 'Yakutiye']
  },
  {
    name: 'Van',
    districts: ['Bahçesaray', 'Başkale', 'Çaldıran', 'Çatak', 'Edremit', 'Erciş', 'Gevaş', 'Gürpınar', 'İpekyolu', 'Muradiye', 'Özalp', 'Saray', 'Tuşba']
  },
  {
    name: 'Sivas',
    districts: ['Akıncılar', 'Altınyayla', 'Divriği', 'Doğanşar', 'Gemerek', 'Gölova', 'Gürün', 'Hafik', 'İmranlı', 'Kangal', 'Koyulhisar', 'Merkez', 'Suşehri', 'Şarkışla', 'Ulaş', 'Yıldızeli', 'Zara']
  },
  {
    name: 'Çanakkale',
    districts: ['Ayvacık', 'Bayramiç', 'Biga', 'Bozcaada', 'Çan', 'Eceabat', 'Ezine', 'Gelibolu', 'Gökçeada', 'Lapseki', 'Merkez', 'Yenice']
  },
  { name: 'Afyonkarahisar', districts: ['Merkez', 'Bolvadin', 'Çay', 'Dinar', 'Emirdağ', 'Sandıklı', 'Şuhut'] },
  { name: 'Ağrı', districts: ['Merkez', 'Doğubayazıt', 'Diyadin', 'Eleşkirt', 'Patnos'] },
  { name: 'Amasya', districts: ['Merkez', 'Merzifon', 'Suluova', 'Taşova'] },
  { name: 'Artvin', districts: ['Merkez', 'Hopa', 'Borçka', 'Arhavi', 'Yusufeli'] },
  { name: 'Bilecik', districts: ['Merkez', 'Bozüyük', 'Osmaneli', 'Söğüt'] },
  { name: 'Bingöl', districts: ['Merkez', 'Genç', 'Karlıova', 'Solhan'] },
  { name: 'Bitlis', districts: ['Merkez', 'Tatvan', 'Ahlat', 'Güroymak'] },
  { name: 'Bolu', districts: ['Merkez', 'Gerede', 'Mengen', 'Mudurnu'] },
  { name: 'Burdur', districts: ['Merkez', 'Bucak', 'Gölhisar', 'Yeşilova'] },
  { name: 'Çankırı', districts: ['Merkez', 'Çerkeş', 'Ilgaz', 'Kurşunlu'] },
  { name: 'Çorum', districts: ['Merkez', 'Sungurlu', 'Osmancık', 'Alaca'] },
  { name: 'Edirne', districts: ['Merkez', 'Keşan', 'Uzunköprü', 'İpsala'] },
  { name: 'Elazığ', districts: ['Merkez', 'Kovancılar', 'Karakoçan', 'Palu'] },
  { name: 'Erzincan', districts: ['Merkez', 'Tercan', 'Üzümlü', 'Refahiye'] },
  { name: 'Giresun', districts: ['Merkez', 'Bulancak', 'Espiye', 'Görele', 'Tirebolu'] },
  { name: 'Gümüşhane', districts: ['Merkez', 'Kelkit', 'Şiran', 'Torul'] },
  { name: 'Hakkari', districts: ['Merkez', 'Yüksekova', 'Şemdinli', 'Çukurca'] },
  { name: 'Isparta', districts: ['Merkez', 'Yalvaç', 'Eğirdir', 'Şarkikaraağaç'] },
  { name: 'Kars', districts: ['Merkez', 'Kağızman', 'Sarıkamış', 'Digor'] },
  { name: 'Kastamonu', districts: ['Merkez', 'Tosya', 'Taşköprü', 'Cide', 'İnebolu'] },
  { name: 'Kırklareli', districts: ['Merkez', 'Lüleburgaz', 'Babaeski', 'Vize'] },
  { name: 'Kırşehir', districts: ['Merkez', 'Kaman', 'Mucur', 'Çiçekdağı'] },
  { name: 'Kütahya', districts: ['Merkez', 'Tavşanlı', 'Simav', 'Gediz'] },
  { name: 'Mardin', districts: ['Artuklu', 'Kızıltepe', 'Midyat', 'Nusaybin', 'Derik'] },
  { name: 'Muş', districts: ['Merkez', 'Bulanık', 'Malazgirt', 'Varto'] },
  { name: 'Nevşehir', districts: ['Merkez', 'Ürgüp', 'Avanos', 'Gülşehir', 'Derinkuyu'] },
  { name: 'Niğde', districts: ['Merkez', 'Bor', 'Çamardı', 'Ulukışla'] },
  { name: 'Ordu', districts: ['Altınordu', 'Ünye', 'Fatsa', 'Kumru', 'Korgan'] },
  { name: 'Rize', districts: ['Merkez', 'Çayeli', 'Ardeşen', 'Pazar', 'Fındıklı'] },
  { name: 'Sinop', districts: ['Merkez', 'Boyabat', 'Gerze', 'Ayancık'] },
  { name: 'Tokat', districts: ['Merkez', 'Erbaa', 'Turhal', 'Niksar', 'Zile'] },
  { name: 'Tunceli', districts: ['Merkez', 'Pertek', 'Hozat', 'Mazgirt'] },
  { name: 'Uşak', districts: ['Merkez', 'Banaz', 'Eşme', 'Sivaslı'] },
  { name: 'Yozgat', districts: ['Merkez', 'Sorgun', 'Akdağmadeni', 'Yerköy', 'Boğazlıyan'] },
  { name: 'Zonguldak', districts: ['Merkez', 'Ereğli', 'Çaycuma', 'Devrek', 'Kozlu', 'Kilimli'] },
  { name: 'Aksaray', districts: ['Merkez', 'Ortaköy', 'Eskil', 'Gülağaç'] },
  { name: 'Bayburt', districts: ['Merkez', 'Demirözü', 'Aydıntepe'] },
  { name: 'Karaman', districts: ['Merkez', 'Ermenek', 'Sarıveliler'] },
  { name: 'Kırıkkale', districts: ['Merkez', 'Yahşihan', 'Keskin', 'Delice'] },
  { name: 'Batman', districts: ['Merkez', 'Kozluk', 'Sason', 'Beşiri', 'Gercüş'] },
  { name: 'Şırnak', districts: ['Merkez', 'Cizre', 'Silopi', 'İdil', 'Uludere'] },
  { name: 'Bartın', districts: ['Merkez', 'Amasra', 'Ulus', 'Kurucaşile'] },
  { name: 'Ardahan', districts: ['Merkez', 'Göle', 'Çıldır', 'Hanak'] },
  { name: 'Iğdır', districts: ['Merkez', 'Tuzluca', 'Aralık', 'Karakoyunlu'] },
  { name: 'Yalova', districts: ['Merkez', 'Çiftlikköy', 'Çınarcık', 'Altınova', 'Armutlu'] },
  { name: 'Karabük', districts: ['Merkez', 'Safranbolu', 'Yenice', 'Eskipazar'] },
  { name: 'Kilis', districts: ['Merkez', 'Musabeyli', 'Elbeyli', 'Polateli'] },
  { name: 'Osmaniye', districts: ['Merkez', 'Kadirli', 'Düziçi', 'Bahçe', 'Toprakkale'] },
  { name: 'Düzce', districts: ['Merkez', 'Akçakoca', 'Kaynaşlı', 'Gölyaka', 'Çilimli'] }
];
